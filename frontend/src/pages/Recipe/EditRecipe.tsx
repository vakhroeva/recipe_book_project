...
import {getCategories, updateRecipe, createIngredients, createSteps, getRecipeById} from '../../api/recipesAPI';
...
const EditRecipe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const recipesData = async () => {
      try {
        const data = await getRecipeById(id, authToken);
        setRecipeData(data.data);
        setLoading(false);

        const mainPhotoUrl = data.data.main_photo_url;
        const previewUrl = `${process.env.REACT_APP_API_URL}${mainPhotoUrl}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recipe = await updateRecipe(id, formData, authToken);
  ...
  return (
    <div className="container mt-4 col-12 col-md-6">
      <h2>Страница редактирования рецепта</h2>
